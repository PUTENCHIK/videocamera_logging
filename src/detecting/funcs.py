...
async def monitor_camera(address: str, camera_id: int):
    ...
    async for db in get_db_session():
        try:
            ...
            while task.running:
                ...
                if current_time - previous_time >= Config.detecting_delay:
                    ...
                    if results.any_objects:
                        snapshot = await _parse_detecting_results(results, camera_id, db)
            ...
            print(f"{address}: CancelledError catched")
...
async def start_camera_monitoring():
    async for db in get_db_session():
        try:
            cameras = await _get_monitoring_cameras(db)
            ...
            print("Start camera monitoring failed:", e)
            db.rollback()
